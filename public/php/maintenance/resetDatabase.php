...
QUERY;
	...
	if (!$result) {
        ...
        data_reject($conn, "400", "executed", "Query failed (clear personnel).");

    }

    /* 4. Delete all from department ===== */
    // Setup query
    $query_string = <<<QUERY
...
QUERY;
	...
	if (!$result) {
        ...
        data_reject($conn, "400", "executed", "Query failed (clear department).");

    }

    /* 5. Delete all from location ===== */
    // Setup query
    $query_string = <<<QUERY
...
QUERY;
	...
	if (!$result) {
        ...
        data_reject($conn, "400", "executed", "Query failed (clear location).");
...
QUERY;
	...
	if (!$result) {
        ...
        data_reject($conn, "400", "executed", "Query failed (reset location).");
...
QUERY;
	...
	if (!$result) {
        ...
        data_reject($conn, "400", "executed", "Query failed (reset department).");
    ...
    $query_string = <<<QUERY
        ...
        VALUES (1, 'Robert', 'Heffron', '', 'rheffron0@ibm.com', 1),
...
QUERY;
    ...
    data_return($conn, "200", "ok", "Database was successfully reset.", $execution_start_time, $data);
